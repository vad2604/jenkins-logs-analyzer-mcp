...
Процесс должен висеть без вывода в stdout (это нормально для stdio MCP). По умолчанию консольные логи отключены для максимальной совместимости с MCP-клиентами; при необходимости включите `ANALYZER_LOG_TO_STDERR=true` или задайте `LOG_FILE`.
...
Замените `ABSOLUTE/PATH/TO/...` на реальные пути на машине коллеги. Если `LOG_FILE` не задан — файл лога не создаётся. Для логов в консоль явно включайте `ANALYZER_LOG_TO_STDERR=true`.
...
| Переменные | `LOG_FILE` — полный путь к файлу; `ANALYZER_LOG_TO_STDERR=true` — включить логи в stderr |
| Stdout | **Не используется** для логов (занят протоколом MCP) |
| Stderr | По умолчанию выключен; включается через `ANALYZER_LOG_TO_STDERR=true` |
